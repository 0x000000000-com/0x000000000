...
const dict = {
  zh: {
    ...
    noteProto: '原型说明：本页已对接真后端（订单/签名验证/铸造/交付）；仅支付为模拟。s 的生成与签名由本地工具完成。',
    // LIVEPAY0X：切了真收款之后上面那句就是假话了。liveSwap 会用下面这句顶替它。
    noteProtoLive: '本页对接的是真后端：下单、签名验证、铸造、交付、以及【真实链上收款】。s 的生成与签名全部在你自己的浏览器/电脑上完成，平台看不到。',
    ...
    proof1: '你的那一半钥匙在你自己的浏览器里生成，只存在你自己的电脑上。平台收到的只有公钥（公开的，像银行卡号）和签名（证明你手里有那一半，但不暴露它）—— 下面「发给平台的全部内容」逐条列着，你自己看。',
    ...
    proofVer: '页面版本',
    proofDl: '下载这个页面到自己电脑上用',
    proofGh: '到 GitHub 核对',
    sentHead: '这一页发给平台的全部内容',
    sentNone: '还没有发过任何东西。',
    sentCheck: (n, sHit, kHit, hasS, hasK) => '一共 ' + n + ' 次。' + (hasS ? '你的那一半钥匙在里面出现 ' + sHit + ' 次' : '你还没造钥匙') + '；' + (hasK ? '完整私钥出现 ' + kHit + ' 次。' : '完整私钥还没合出来。'),
    sentRaw: '原样内容',
    sent: {
      catalog: '读价目表 —— 什么都没发',
      health: '看平台在不在线 —— 什么都没发',
      create: '下单 —— 发了：哪条链、图案、用哪条链付款',
      share: '发了公钥 A（公开的）和一个签名（证明你手里有那一半，不暴露它）',
      payment: '要收款地址 —— 什么都没发',
      status: '查订单进度 —— 什么都没发',
      paysim: '测试用的模拟付款 —— 什么都没发',
      download: '取货 —— 发了一个签名（证明是你本人来取）',
      receipt: '交收条 —— 发了一个签名',
      other: '其他请求',
    },
    foot2: '平台永远不会索要你的 s 或私钥——任何索要私钥的消息都是钓鱼。',
    ...
    flow: {
      ...
      prePh: '开头', sufPh: '结尾',
      s1head: '造一把只有你有的钥匙',
      s1desc: '这一步全部在你自己的浏览器里算完，我们一个字节都收不到。它会把一把钥匙劈成两半：一半留在你这儿（叫 s），另一半交给我们（叫影子 A）。我们拿着 A 也推不回 s —— 所以从头到尾，完整私钥只可能出现在你的电脑上。',
      genBtn: '造钥匙',
      genOk: '钥匙造好了。备份文件已经下载：my-secret-s.json',
      ...
      s2head: '下单',
      s2desc: '点一下就好，下面三件事页面自己做完：① 建单，拿到我们给的一句随机的话；② 用你刚才那半把钥匙给它签个名，证明 s 真在你手上（防止有人白嫖我们的算力）；③ 把影子 A 和签名交上来。签名也是在你的浏览器里算的。',
      create: '下单',
      created: '订单建好了，编号',
      ...
      s5head: '取货：把两半合起来',
      s5desc: '我们把另外半把钥匙（叫 b）交给你。下面这一下会在你的浏览器里把 s 和 b 合成完整私钥，并且当场核对算出来的地址跟你买的那个一不一样 —— 对不上就不让你往下导出。',
      dlBtn: '取货并合成我的钥匙',
      gotB: '拿到 b 了，正在本机合成…',
      ...
      ksOkTron: '导好了：my-keystore.json —— 这是加密备份，留好。TRON 地址不要导进 MetaMask（它不认 TRON，只会显示同一把钥匙的 0x 写法）。导进 TRON 钱包（如 TronLink）请用下面「导出明文私钥」，选「导入私钥」，导完立刻删掉那个文件。',
      ksFail: '加密失败：',
      plainWarn: '明文私钥已下载（my-key.txt）。任何人拿到这个文件就拿到了这个地址 —— 导进钱包后请立刻删掉它。',

      s6head: '交个收条',
      s6desc: '用刚合成的完整钥匙给一句话签个名交回来。我们手上没有你的 s，这个签名我们自己造不出来 —— 收到就等于双方都认这一单交付完成了。',
      receiptBtn: '签收条并提交',
  ...
  en: {
    ...
    noteProto: 'Prototype note: this page is wired to the real backend (order / signature verification / minting / delivery); only payment is simulated. s generation and signing happen in the local tools.',
    noteProtoLive: 'This page is wired to the real backend: ordering, signature verification, minting, delivery, and real on-chain payment. s is generated and signed only on your own machine.',
    ...
    heroCta: 'Start an order →',
    heroTool: 'Offline key tool ↗',
    toolCta: 'Do this step in your browser (nothing to install) ↗',
    foot1: 'page code published on GitHub, verifiable byte for byte · custom zone non-custodial · USDT on TRON / BSC',
    proof1: 'Your half of the key is generated in your own browser and only ever stored on your own computer. The platform only receives a public key (public, like a bank account number) and signatures (proof that you hold your half, without revealing it). Every single thing this page sends is listed below under "Everything this page sent" - check it yourself.',
    ...
    proofDl: 'Download this page and use it on your own computer',
    proofGh: 'Verify on GitHub',
    sentHead: 'Everything this page sent to the platform',
    sentNone: 'Nothing sent yet.',
    sentCheck: (n, sHit, kHit, hasS, hasK) => n + ' requests. ' + (hasS ? 'Your half of the key appears ' + sHit + ' times' : 'No key made yet') + '; ' + (hasK ? 'the full private key appears ' + kHit + ' times.' : 'full key not assembled yet.'),
    sentRaw: 'raw',
    sent: {
      catalog: 'read the price list - nothing sent',
      health: 'checked the platform is online - nothing sent',
      create: 'order - sent: chain, pattern, payment chain',
      share: 'sent public key A (public) and a signature (proves you hold your half without revealing it)',
      payment: 'asked for the payment address - nothing sent',
      status: 'checked order progress - nothing sent',
      paysim: 'test-only simulated payment - nothing sent',
      download: 'collect - sent a signature (proves it is you)',
      receipt: 'receipt - sent a signature',
      other: 'other request',
    },
    foot2: 'The platform will never ask for your s or private key — any message asking for keys is phishing.',
    ...
    flow: {
      ...
      s1desc: 'This runs entirely in your own browser; we receive none of it. It splits one key in two: one half stays with you (called s), the other goes to us (called shadow A). A cannot be turned back into s, so the complete private key only ever exists on your machine.',
      genBtn: 'Make my key',
      genOk: 'Key created. Your backup has been downloaded: my-secret-s.json',
      ...
      s2desc: 'One click. The page does three things for you: creates the order and receives a random phrase from us; signs that phrase with your half of the key to prove you really hold s (this stops people burning our machines for free); and submits shadow A plus that signature. The signing also happens in your browser.',
      create: 'Place order',
      ...
      s5desc: 'We hand you the other half (called b). The button below joins s and b into the complete private key inside your browser, and checks the resulting address against the one you bought - if they differ, it will not let you export.',
      dlBtn: 'Collect and build my key',
      gotB: 'Got b - joining locally...',
      ...
      ksOkTron: 'Exported: my-keystore.json - keep it as your encrypted backup. Do not import a TRON address into MetaMask (it does not support TRON and would only show the 0x form of the same key). For a TRON wallet such as TronLink, use "Export plain private key" below, choose import by private key, then delete that file right away.',
      ksFail: 'Encryption failed: ',
      plainWarn: 'Plain private key downloaded (my-key.txt). Anyone who gets this file owns the address - delete it right after importing.',

      s6head: 'Send a receipt',
      s6desc: 'Sign one phrase with the key you just built. We do not hold your s, so we could not forge this signature - receiving it means both sides agree the order was delivered.',
...
// CLIREACH/BROWSERKEY_20260922: 浏览器版密钥工具入口（tool.html，不联网、无需装 node）
// ★★★★ ENTRYFLIP_20260922 —— 把默认那条路翻过来。
//   DSJ 20260922 原话：「新的UI我没看见你上线啊？」。上线的是【文件】，没上线的是【入口】：
//   首页唯一一个链接在 <footer>（全页 6208 字节里的第 5678 字节 = 91% 处），
//   而流程里这个入口挂在一条 PowerShell 命令【后面】—— 页面仍然先叫人复制命令。
//   他的死令是「如果中间你发代码给我叫我贴，我是不会管你的」。台账 E24：我做了 ≠ 他看得见。
//   所以：浏览器按钮在前，命令行降成「或者用命令行：」。
const toolink = () => '<a class="toolink" data-i18n="toolCta" href="tool.html" target="_blank" rel="noopener">' + esc(t('toolCta')) + '</a>';
// CLI404_20260923：命令行脚本还没发布（四个全 404），所以只给浏览器工具那条路。
//   cmd 这个参数先留着 —— 脚本真发出去的那天,把下面那行注释掉的写法换回来即可。
const altcmd = (cmd) => { void cmd; return toolink(); };
...
function renderSent() {
  ...
  for (const x of SENT) {
    ...
    if (last && last.kind === x.kind && x.kind === 'status') { last.n++; continue; }
    rows.push({ kind: x.kind, n: 1, raw: x.method + ' ' + x.url + (x.body ? '\n' + x.body : '') });
  }
  list.innerHTML = rows.map((r) => '<li>' + esc((f[r.kind] || f.other) + (r.n > 1 ? '  ×' + r.n : '')) +
    '<details><summary>' + esc(t('sentRaw')) + '</summary><pre>' + esc(r.raw) + '</pre></details></li>').join('');
...
async function api(path, method = 'GET', body) {
  SENT.push({ method, url: path, kind: sentKind(method, path), body: body ? JSON.stringify(body) : '' });
...
function showApiError(container, res) {
  ...
  outLine(container, 'HTTP ' + res.code + ' ' + (b.error || ''), 'err');
...
function renderSteps() {
  ...
  // ── [1/6] 造钥匙 ────────────────────────────────────────────────────
  const li1 = stepBox(1, f.s1head, f.s1desc);
  li1.innerHTML +=
  ...
  // ── [2/6] 下单 ──────────────────────────────────────────────────────
  const li2 = stepBox(2, f.s2head, f.s2desc);
  li2.innerHTML +=
  ...
  // ── [3/6] 付款 ──────────────────────────────────────────────────────
  const li3 = stepBox(3, f.s3head, IS_LIVE ? f.s3descLive : f.s3desc);
  li3.innerHTML +=
  ...
  // ── [4/6] 铸造 ──────────────────────────────────────────────────────
  const li4 = stepBox(4, f.s4head, f.s4desc);
  li4.innerHTML += '<pre class="step-out" id="out4" hidden></pre>';
  stepsEl.appendChild(li4);

  // ── [5/6] 取货 + 当场合成 ───────────────────────────────────────────
  const li5 = stepBox(5, f.s5head, f.s5desc);
  li5.innerHTML +=
    '<div class="flow-ctl"><button class="run" id="btnDownload" disabled>' + esc(f.dlBtn) + '</button></div>' +
  ...
  // ── [6/6] 回执 ──────────────────────────────────────────────────────
  const li6 = stepBox(6, f.s6head, f.s6desc);
  li6.innerHTML +=
...
function wireOrderFlow(f) {
  const chainMeta = () => (CATALOG && CATALOG.chains || []).find((c) => c.id === PICKED) || null;
  ...
  function renderChainPick() {
    const box = $('#chainPick'); if (!box || !CATALOG || !CATALOG.chains) return;
    box.innerHTML = CATALOG.chains.map((c) =>
  ...
  function syncPosOptions() {
    const sel = $('#orderPos'); if (!sel || !CATALOG) return;
    const ok = (kind) => CATALOG.rows.some((r) => (r.chain || 'evm') === PICKED && r.kind === kind && r.orderable);
  ...
  else { loadCatalog().then(bootOrder).catch(() => { renderChainPick(); renderPayChains(); refreshHint(); }); }
  ...
  $('#btnDownload').addEventListener('click', async () => {
    ...
    if (res.code !== 200) { showApiError(out, res); return; }
    orderState.b = res.body.b;
    outLine(out, f.gotB, 'dim');
    let r;
    try { r = KC().merge(orderState.sec, KC().parseB(res.body.b)); }
    catch (e) { return outLine(out, e.message, 'err'); }
    if (!r.ok) {
    ...
    outLine(out, r.addr === res.body.address ? f.addrMatch : f.addrDiff(res.body.address), r.addr === res.body.address ? 'ok' : 'err');
    $('#ksBox').hidden = false;
    setStatus('delivered');
  ...
  $('#btnPlain').addEventListener('click', () => {
    if (!MERGED) return;
    dl('my-key.txt', '0x' + KC().pad64(MERGED.k));
    const msg = $('#ksMsg'); msg.className = 'step-desc warn'; msg.textContent = f.plainWarn;
  });
...
renderSteps();
renderBoot(false);
// 带着订单号进来（比如 track.html 跳过来,或者他自己存的链接）就直接续
try {
  const q = new URLSearchParams(location.search).get('order');
  if (q) setTimeout(() => resumeFrom(q, $('#resumeMsg')), 600);
} catch (e) {}
